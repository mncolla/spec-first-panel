...
import type { OperatorWrite } from '../types.ts'

type Props = {
  pending: boolean
  apiError?: string
  onSubmit: (body: OperatorWrite) => void
}

export function CreateAgentForm({ pending, apiError, onSubmit }: Props) {
  ...
  function handleSubmit(event: FormEvent) {
    ...
    const session = sessionWriteFromDraft(draft)
    onSubmit({ email: session.email, clave: session.clave, rol: 'agente' })
  }

  return (
    <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2 md:items-end" noValidate>
      ...
      {apiError ? (
        <div role="alert" className="sheet px-4 py-3 text-danger md:col-span-2">
          {apiError}
        </div>
      ) : null}
      <button
